...
def CreateADF(rg_name,df_name,df_params,adf_client):
  adf_identity = FactoryIdentity(type='SystemAssigned',principal_id=CLIENT_ID,tenant_id=TENANT_ID)
...
def CreateAzureKeyVaultLinkedService(rg_name, df_name, AKV_LS_NAME,keyvaultName,adf_client):
  ls_azure_storage = AzureKeyVaultLinkedService(type='str',base_url='https://' + keyvaultName +'.vault.azure.net/')
  ls = adf_client.linked_services.create_or_update(rg_name, df_name, AKV_LS_NAME, ls_azure_storage)
  print_item(ls)
...
def CreateAzureDataLakeStorageLinkedService(rg_name, df_name, ls_name,ADLSURI,adf_client,AKVsecret_name,AzureKVreference_name):
  lsr=LinkedServiceReference(type = 'LinkedServiceReference',reference_name= AzureKVreference_name)
  account_key= AzureKeyVaultSecretReference(type ='str',store=lsr, secret_name=AKVsecret_name)
  ls_azure_storage = AzureDataLakeStoreLinkedService(data_lake_store_uri=ADLSURI ,service_principal_id=CLIENT_ID,service_principal_key=account_key, tenant=TENANT_ID)
...
def CreateParquetDataset(rg_name, df_name, ds_name,DATASET_FLODER_NAME,FOLDER_PATH,FILE_NAME,adf_client,ls_name,compression_TYPE='none'):
  ...
  DatasetLocation_adls=AzureDataLakeStoreLocation(folder_path=FOLDER_PATH,file_name=FILE_NAME)
...
def CreateCopyActivity(Activity_Name,Sourceadls, Sinkadls):
  ...
  copy_activity = CopyActivity(name=Activity_Name, inputs=[Sourcein_ref], outputs=[SinkOut_ref], source=ADLS_source, sink=ADLS_sink)
  return copy_activity

def CreatePipeline(adf_client,rg_name,df_name,p_name,Activity):
  params_for_pipeline = {}
  print('start creating pipeline object')
  p_obj = PipelineResource(activities=Activity, parameters=params_for_pipeline)
  print('start creating pipeline with copy activity')
  print(p_obj)
...
def DynamicADF(rg_name,df_name,subscription_id,TENANT_ID,CLIENT_ID,keyvaultName,AKVsecret_name,Source_ADLSURI,Sink_ADLSURI,SRC_FOLDER_PATH,SINK_FOLDER_PATH,FILES_NAME=[]):
  ...
  AzureKeyVaultLinkedService, adf_client =CreateAzureKeyVaultLinkedService(rg_name, df_name, 'AzureKeyVaultLinkedService',keyvaultName,adf_client)

  AzureDataLakeStorageLinkedService, adf_client = CreateAzureDataLakeStorageLinkedService(rg_name,df_name,'AzureDataLakeStorageLinkedService',Source_ADLSURI,adf_client,AKVsecret_name,'AzureKeyVaultLinkedService')
  
  print("Fille list are\n",FILES_NAME)
  
  avtivity=[]
  for FILE_NAME in FILES_NAME:
    print("crate datasets for\n",FILES_NAME)
    
    SourceprqDataset, adf_client=CreateParquetDataset(rg_name,df_name,FILE_NAME+'SourceprqDataset','staging',SRC_FOLDER_PATH,FILE_NAME,adf_client,'AzureDataLakeStorageLinkedService',compression_TYPE='none')
  
    SinkprqDataset, adf_client=CreateParquetDataset(rg_name,df_name,FILE_NAME+'SinkprqDataset','staging',SINK_FOLDER_PATH,FILE_NAME,adf_client,'AzureDataLakeStorageLinkedService',compression_TYPE='none')                 
    FILE_NAME = CreateCopyActivity(FILE_NAME,FILE_NAME+'SourceprqDataset',FILE_NAME+'SinkprqDataset')
    avtivity.append(FILE_NAME)
    print(avtivity)
  pipeline, adf_client = CreatePipeline(adf_client,rg_name,df_name,'CopyAdlsToAdls',avtivity)
...
df_name = 'testadfnew02'
subscription_id = '8706626e-72cb-4bc5-9e7c-ebfe52d0c2ba'
...
Source_ADLSURI='adl://stagingadls.azuredatalakestore.net'
Sink_ADLSURI='adl://stagingadls.azuredatalakestore.net'
SRC_FOLDER_PATH = ''
SINK_FOLDER_PATH = 'Output'
FILES_NAME= ["Test1","Test"]
